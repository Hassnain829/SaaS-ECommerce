<section class="rounded-2xl border border-[#CBD5E1] bg-white shadow-sm">
    <div class="flex flex-col gap-3 border-b border-[#F1F5F9] px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
            ...
            <p class="mt-1 text-sm text-[#64748B]">Checkout options such as Standard delivery, Express, Local delivery, or Store pickup.</p>
        </div>
        @if ($canManageShipping && $shippingZones->isNotEmpty())
            <button type="button" data-open-drawer="method-add" class="inline-flex h-10 items-center rounded-lg bg-[#0052CC] px-4 text-sm font-bold text-white">Add delivery method</button>
        @endif
    </div>
            ...
            <article class="flex flex-col gap-3 p-5 md:flex-row md:items-start md:justify-between">
                <div class="min-w-0">
                    <div class="flex flex-wrap items-center gap-2">
                        ...
                        @if ($method->enabled_for_checkout)<span class="rounded-full bg-[#EFF6FF] px-2.5 py-1 text-xs font-bold text-[#1D4ED8]">Checkout</span>@endif
                    </div>
                    <p class="mt-2 text-sm text-[#64748B]">
                        Zone: {{ $method->shippingZone?->name ?? '—' }}
                        · {{ $rateLabels[$method->rate_type] ?? $method->rate_type }}
                        · Carrier: {{ $method->carrierAccount?->display_name ?? 'None' }}
                    </p>
                ...
                @if ($canManageShipping)
                    <div class="flex shrink-0 flex-wrap gap-2">
                        <button type="button" class="method-edit-btn rounded-lg border border-[#CBD5E1] bg-white px-3 py-2 text-xs font-semibold text-[#475569]"
                            ...
                            data-active="{{ $method->is_active ? '1' : '0' }}">Edit</button>
