<section class="rounded-2xl border border-[#CBD5E1] bg-white shadow-sm">
    <div class="flex flex-col gap-3 border-b border-[#F1F5F9] px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
            <h2 class="text-xl font-poppins font-semibold text-[#0F172A]">Delivery zones</h2>
            <p class="mt-1 text-sm text-[#64748B]">Define where this store delivers. Methods match the customer's address to a zone.</p>
        </div>
        @if ($canManageShipping)
            <button type="button" data-open-drawer="zone-add" class="inline-flex h-10 items-center rounded-lg bg-[#0052CC] px-4 text-sm font-bold text-white">Add zone</button>
        @endif
    </div>
                            ...
                            <td class="px-5 py-4 text-right">
                                <div class="flex flex-wrap justify-end gap-2">
                                    <button type="button" class="zone-edit-btn rounded-lg border border-[#CBD5E1] bg-white px-3 py-1.5 text-xs font-semibold text-[#475569]"
                                        ...
                                        data-name="{{ $zone->name }}"
                                        data-countries="{{ collect($zone->countries)->filter()->implode(', ') }}"
                                        data-regions="{{ collect($zone->regions)->filter()->implode(', ') }}"
                                        data-postal="{{ collect($zone->postal_patterns)->filter()->implode(', ') }}"
                                        data-sort="{{ $zone->sort_order }}"
                                        data-active="{{ $zone->is_active ? '1' : '0' }}"
                                        data-action="{{ route('settings.shipping.zones.update', $zone) }}">Edit</button>
