            ...
            <form id="editProductForm" method="POST" enctype="multipart/form-data" class="space-y-6">
                ...
                @if ($productEditSurface === 'page' && $productEditPageNative)
                    <nav id="catalog-editor-section-nav" class="flex flex-wrap gap-2 rounded-2xl border border-slate-200/80 bg-white px-3 py-2.5 text-xs font-semibold text-[#475569] shadow-sm" aria-label="Jump to editor sections">
                        ...
                        <a href="#catalog-edit-section-attributes" class="rounded-lg px-2.5 py-1 hover:bg-[#F1F5F9]">Attributes</a>
                ...
                <div class="{{ $editSectionClass }}" @if ($productEditSurface === 'page' && $productEditPageNative) id="catalog-edit-section-basics" @endif>
                    ...
                    <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                        ...
                        <div>
                            ...
                            <p id="editProductTypeBehaviorHelp" class="mt-2 text-xs text-[#64748B]">Fulfillment and stock behavior, not the catalog category.</p>
                        </div>
                    </div>
                    @if ($productEditSurface === 'page' && $productEditPageNative)
                ...
                <div class="{{ $editSectionClass }}" @if ($productEditSurface === 'page' && $productEditPageNative) id="catalog-edit-section-attributes" @endif>
                    <div @class(['mb-4 border-b border-slate-100 pb-4' => $productEditSurface === 'page' && $productEditPageNative, 'mb-4' => ! ($productEditSurface === 'page' && $productEditPageNative)])>
                        <h3 class="text-lg font-semibold text-[#0F172A] font-[Poppins] sm:text-xl scroll-mt-28">Product attributes</h3>
                        <p class="mt-2 text-sm leading-relaxed text-[#64748B]">Attributes are structured product facts shoppers can filter or compare, such as material, capacity, color family, or ingredients. Product options still live under <span class="font-medium text-[#334155]">Option groups</span>.</p>
                    </div>
...
const renderProductAttributeRows=(selectedIds=[])=>{if(!editProductAttributesBody)return;const selected=new Set((Array.isArray(selectedIds)?selectedIds:[]).map((id)=>String(id)));if(!catalogAttributeDefs.length){editProductAttributesBody.innerHTML='<div class="rounded-xl border border-dashed border-[#CBD5E1] bg-[#F8FAFC] px-4 py-4 text-sm text-[#64748B]"><p class="font-medium text-[#334155]">No attributes have been created for this store yet.</p><p class="mt-1">Create attributes from Catalog tools, then return here to select terms for this product.</p></div>';return;}editProductAttributesBody.innerHTML=catalogAttributeDefs.map((attr)=>{const terms=Array.isArray(attr.terms)?attr.terms:[];const termHtml=terms.length?terms.map((term)=>{const checked=selected.has(String(term.id))?' checked':'';const swatch=term.swatch_value?'<span class="h-3 w-3 rounded-full border border-slate-200" style="background:'+escapeHtml(term.swatch_value)+'"></span>':'';return '<label class="inline-flex items-center gap-2 rounded-lg border border-[#E2E8F0] bg-white px-3 py-2 text-sm text-[#334155]"><input type="checkbox" name="attribute_terms['+escapeHtml(attr.id)+'][]" value="'+escapeHtml(term.id)+'" class="rounded border-[#CBD5E1] accent-[#0052CC]"'+checked+'>'+swatch+'<span>'+escapeHtml(term.name)+'</span></label>';}).join(''):'<p class="text-sm text-[#94A3B8]">No terms yet. Add terms on the attributes page before assigning this attribute.</p>';return '<div class="rounded-xl border border-[#E2E8F0] bg-[#F8FAFC] p-4"><div class="flex flex-wrap items-center gap-2"><p class="text-sm font-semibold text-[#0F172A]">'+escapeHtml(attr.name)+'</p>'+(attr.is_filterable?'<span class="rounded-full bg-[#EEF4FF] px-2 py-0.5 text-[10px] font-bold uppercase tracking-wide text-[#0052CC]">Filterable</span>':'')+'</div><div class="mt-3 flex flex-wrap gap-2">'+termHtml+'</div></div>';}).join('');};
...
const syncType=()=>{const type=defaultTypes.includes(editTypeSelect.value)?editTypeSelect.value:'physical';editTypeSelect.value=type;editTypeValue.value=type;if(editCustomTypeHidden)editCustomTypeHidden.value='';if(editTypeBehaviorHelp){const behavior=productTypeBehaviors[type]||{};editTypeBehaviorHelp.textContent=behavior.description||'Fulfillment and stock behavior, not the catalog category.';}};
