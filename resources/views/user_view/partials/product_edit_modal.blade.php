    ...
    <div @class([
        ...
        <div @class([
            ...
            <form id="editProductForm" method="POST" enctype="multipart/form-data" class="space-y-6" @if ($editFormAction) action="{{ $editFormAction }}" @endif data-product-store-url="{{ route('product.store') }}" data-product-update-url-template="{{ route('product.update', ['productId' => '__PRODUCT_ID__']) }}">
                ...
                <div class="{{ $editSectionClass }}" @if ($isPageNative) id="catalog-edit-section-basics" data-product-edit-section @endif>
                    ...
                    <div class="{{ $editSectionClass }}" id="catalog-edit-section-media" data-product-edit-section>
                        <div class="mb-6 border-b border-slate-100 pb-4">
                            ...
                            <p class="mt-1 text-xs text-[#64748B]">Add listing photos here. The first photo is what shoppers see first. You can assign a photo to each option later.</p>
                        </div>
                    @endif
                    <div @class(['grid grid-cols-1 gap-6' => $isPageNative, 'mt-4 grid grid-cols-1 gap-6' => ! $isPageNative])>
                        <div class="product-image-uploader" data-product-image-uploader>
                            ...
                            >
                                ...
                                <p class="product-image-dropzone-help">JPG, PNG, or WebP. You can add more than one.</p>
                    ...
                    <div class="{{ $editSectionClass }}" id="catalog-edit-section-pricing" data-product-edit-section>
                        <div class="mb-6 border-b border-slate-100 pb-4">
                            <h3 class="text-lg font-semibold text-[#0F172A] sm:text-xl">Price &amp; inventory</h3>
                            <p class="mt-1 text-xs text-[#64748B]">Set the selling price and how many units you have. For products with size or color options, stock is set on each option below.</p>
                        </div>
                    @endif
                    <div @class(['grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-4' => $isPageNative, 'mt-6 grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-4' => ! $isPageNative]) @unless ($isPageNative) id="catalog-edit-section-pricing" @endunless>
                        ...
                        <div><label for="edit_product_price" class="mb-2 block text-sm font-medium text-[#334155]">Price</label><input id="edit_product_price" name="base_price" type="number" min="0" step="0.01" value="{{ old('base_price', '') }}" class="w-full rounded-lg border border-[#CBD5E1] px-4 py-3 text-sm text-[#0F172A]"><p class="mt-1 text-xs text-[#64748B]">Selling price. Option combinations can use a different price.</p></div>
                        ...
                        <div id="editMultiStockHint" class="hidden rounded-lg border border-[#D8E8E1] bg-[#F4FBF8] px-4 py-3">
                            <p class="text-sm font-medium text-[#0A4335]">Stock is set per option</p>
                            <p class="mt-1 text-xs text-[#64748B]">Scroll to <span class="font-medium">Inventory</span> and enter quantity on each row.</p>
                        </div>
                        ...
                        <div id="editMultiStockAlertHint" class="hidden rounded-lg border border-[#E2E8F0] bg-[#F8FAFC] px-4 py-3">
                            ...
                            <p class="mt-1 text-xs text-[#64748B]">Set a low stock alert on each option row in Inventory.</p>
                        </div>
                        ...
                        <div id="editShippingWeightWrap" class="md:col-span-3 hidden rounded-xl border border-[#E2E8F0] bg-white p-4">
                            ...
                            <p class="mt-1 text-xs text-[#64748B]">Used for carrier rates and labels. Leave blank only if this product does not ship.</p>
                            <div class="mt-3 max-w-xs">
                                <label for="edit_product_shipping_weight" class="mb-2 block text-sm font-medium text-[#334155]">Shipping weight</label>
                                <input id="edit_product_shipping_weight" name="shipping_weight" type="number" min="0.01" step="0.01" value="{{ old('shipping_weight', '') }}" class="w-full rounded-lg border border-[#CBD5E1] px-4 py-3 text-sm text-[#0F172A]" inputmode="decimal" autocomplete="off">
                                <p class="mt-1 text-xs text-[#64748B]">Weight of one unit when packed for shipping.</p>
                            </div>
                        </div>
                        @unless ($isPageNative)
                    ...
                    <div class="{{ $editSectionClass }}" id="catalog-edit-section-organization" data-product-edit-section>
                        ...
                        <div class="grid grid-cols-1 gap-6">
                            @if ($catalogTaxonomyCategories->isNotEmpty())
                                <div class="rounded-xl border border-[#CCFBF1]/80 bg-[#F0FDFA]/40 p-4">
                                    <label for="edit_product_category_ids" class="mb-2 block text-sm font-semibold text-[#0F766E]">Catalog categories</label>
                                    <select id="edit_product_category_ids" name="category_ids[]" multiple size="5" class="w-full rounded-lg border border-[#99F6E4]/60 bg-white px-3 py-2 text-sm text-[#0F172A]">
                                        @foreach ($catalogTaxonomyCategories as $catOption)
                                            <option value="{{ $catOption->id }}" @selected(collect(old('category_ids', []))->contains($catOption->id))>{{ $catOption->name }}</option>
                ...
                <div class="{{ $editSectionClass }}" @if ($isPageNative) id="catalog-edit-section-attributes" data-product-edit-section @endif>
                    <div @class(['mb-4 border-b border-slate-100 pb-4' => $isPageNative, 'mb-4' => ! $isPageNative])>
                        ...
                        <p class="mt-2 text-sm leading-relaxed text-[#64748B]">Specifications are reusable product facts shoppers can filter or compare, such as material, capacity, color family, ingredients, or compatibility. Product options still live under <span class="font-medium text-[#334155]">Options</span>.</p>
                    </div>
                ...
                <div class="{{ $editSectionClass }}" @if ($isPageNative) id="catalog-edit-section-option-groups" data-product-edit-section @endif>
                    <div class="mb-6 flex items-center justify-between gap-3">
                        <div>
                            <h3 class="text-lg font-semibold text-[#0F172A] sm:text-xl scroll-mt-28">{{ $isPageNative ? 'Options' : 'Option groups' }}</h3>
                            <p class="mt-1 text-xs text-[#64748B]">Add options like Size or Color only when shoppers choose between different versions. Otherwise keep one inventory row.</p>
                        </div>
                        <button id="editOpenVariationModal" type="button" class="inline-flex shrink-0 items-center gap-2 rounded-full border border-[#D4E3FF] bg-[#EEF4FF] px-4 py-2 text-sm font-semibold text-[#0052CC] transition hover:bg-[#E4EEFF]">Add option</button>
                    </div>
                ...
                <div class="{{ $editSectionClass }}" @if ($isPageNative) id="catalog-edit-section-inventory" data-product-edit-section @endif>
                    ...
                    @if ($isPageNative)
                        <p class="mb-3 text-xs text-[#64748B]">Option photo: pick the same catalog images you added under <span class="font-medium text-[#334155]">Images</span> so each combination can show the right picture.</p>
                    @endif
                    <div id="editInventoryToolsPanel" class="mb-5 hidden space-y-4 rounded-2xl border border-[#E2E8F0] bg-[#F8FAFC] p-4">
                        <div id="editStockCarryNotice" class="hidden rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-950">
                        ...
                        <div>
                            <p class="text-sm font-semibold text-[#0F172A]">Variant inventory tools</p>
                            <p class="mt-1 text-xs leading-relaxed text-[#64748B]">Each variant can have its own price and stock. Use these tools only when you want to set or split stock across all variants at once. Base price above stays the product default.</p>
                        </div>
                        ...
                        <div id="editDistributeStockPanel" class="grid grid-cols-1 gap-3 border-t border-[#E2E8F0] pt-4 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-end">
                            <div>
                                <label for="editDistributeTotal" class="mb-1 block text-xs font-semibold text-[#64748B]">Split total stock across variants</label>
                                <input id="editDistributeTotal" type="number" min="0" step="1" class="h-10 w-full rounded-lg border border-[#E2E8F0] bg-white px-3 text-sm text-[#0F172A]" placeholder="e.g. 90">
                                <p class="mt-1 text-[11px] text-[#64748B]">Divides this total evenly across variant rows. Edit any row afterward for different stock.</p>
                            </div>
                            <button type="button" id="editDistributeEqualBtn" class="inline-flex h-10 items-center justify-center whitespace-nowrap rounded-lg border border-[#BFDBFE] bg-white px-4 text-sm font-semibold text-[#1D4ED8] hover:bg-[#EFF6FF]">Apply equal split</button>
                        </div>
                        <div class="border-t border-[#E2E8F0] pt-3">
                            <button type="button" id="editApplyBasePriceToVariants" class="text-sm font-semibold text-[#0052CC] hover:underline">Use base price on all variants</button>
                        </div>
                    </div>
...
const remappedPendingImageId=(value,removedPendingIndex)=>{const raw=String(value??'');if(!raw.startsWith('new:'))return raw;const idx=Number(raw.slice(4));if(!Number.isFinite(idx))return '';if(removedPendingIndex===null||removedPendingIndex===undefined){return idx>=0&&idx<selectedEditImages.length?raw:'';}if(idx===removedPendingIndex)return '';if(idx>removedPendingIndex)return 'new:'+(idx-1);return raw;};const pruneUnavailableVariantImages=(removedPendingIndex=null)=>{const available=new Set(editCatalogImages.map((img)=>String(img.id)));editRows=editRows.map((row)=>{const nextId=remappedPendingImageId(row.product_image_id,removedPendingIndex);if(nextId===''||!available.has(String(nextId))){return{...row,product_image_id:''};}return nextId===String(row.product_image_id??'')?row:{...row,product_image_id:nextId};});};const labelVariantImageOption=(img,idx)=>{if(img&&img.picker_label)return String(img.picker_label);return 'Catalog photo '+(idx+1);};const buildVariantImageOptionsHtml=(selectedId)=>{const selected=String(selectedId??'');return ['<option value="">No variant image</option>',...editCatalogImages.map((img,ix)=>'<option value="'+escapeHtml(String(img.id))+'" '+(selected===String(img.id)?'selected':'')+'>'+escapeHtml(labelVariantImageOption(img,ix))+'</option>')].join('');};const syncVariantImageHidden=(rowIndex)=>{if(!editVariantRows||!editRows[rowIndex])return;const hidden=editVariantRows.querySelector('input[type="hidden"][data-variant-image-hidden][data-row-index="'+rowIndex+'"]');if(hidden){hidden.value=String(editRows[rowIndex].product_image_id??'');}};const refreshVariantPhotoOptions=()=>{if(!editVariantRows)return;editVariantRows.querySelectorAll('.edit-row-select[data-row-field="product_image_id"]').forEach((select)=>{const r=Number(select.dataset.rowIndex);if(!Number.isFinite(r)||!editRows[r])return;const keep=String(editRows[r].product_image_id??'');const available=editCatalogImages.some((img)=>String(img.id)===keep);const next=available?keep:'';if(!available){editRows[r]={...editRows[r],product_image_id:''};}select.innerHTML=buildVariantImageOptionsHtml(next);select.value=next;syncVariantImageHidden(r);updateVariantPhotoThumb(r);});};const syncEditCatalogImagesFromRetained=()=>{if(!currentProduct)return;const removedPendingIndex=lastRemovedPendingImageIndex;lastRemovedPendingImageIndex=null;const all=Array.isArray(currentProduct.catalog_images)?[...currentProduct.catalog_images]:[];let existing=[];if(retainedExistingImages.length){existing=all.filter((img)=>{const p=img&&img.image_path?String(img.image_path):'';return p!==''&&retainedExistingImages.includes(p);});}const pending=selectedEditImages.map((file,index)=>({id:'new:'+index,image_path:'',thumb_url:URL.createObjectURL(file),picker_label:'New upload — '+file.name}));editCatalogImages=[...existing,...pending];pruneUnavailableVariantImages(removedPendingIndex);if(editVariantRows&&editVariantRows.querySelector('.edit-row-select[data-row-field="product_image_id"]')){refreshVariantPhotoOptions();}else if(editRows&&editRows.length){renderVariantRows();}};
const editImageStatus=document.querySelector('[data-image-upload-status]');const editImageDropzone=document.querySelector('[data-image-dropzone]');const isAcceptedProductImage=(file)=>{if(!file)return false;const name=String(file.name||'');const type=String(file.type||'');return (/^image\/(jpeg|jpg|png|webp)$/i.test(type)||/\.(jpe?g|png|webp)$/i.test(name));};const addEditImageFiles=(fileList)=>{const incoming=[...fileList].filter(isAcceptedProductImage);if(!incoming.length)return;selectedEditImages=[...selectedEditImages,...incoming];syncSelectedFiles(editImageInput,selectedEditImages);renderEditImages();};const syncImageUploadStatus=(count)=>{if(!editImageStatus)return;editImageStatus.textContent=count?count+' photo'+(count===1?'':'s')+'. The first photo is the main listing image.':'No photos yet. The first photo becomes the main listing image.';};const renderEditImages=()=>{const existing=(currentProduct?.image_urls||[]).map((url,index)=>({kind:'existing',url,path:currentProduct?.image_paths?.[index]||''})).filter((item)=>retainedExistingImages.includes(item.path)); const selected=selectedEditImages.map((file,index)=>({kind:'new',url:URL.createObjectURL(file),name:file.name,index})); const items=[...existing,...selected]; syncImageUploadStatus(items.length); if(!editImagePreview){renderExistingImageInputs(); syncEditCatalogImagesFromRetained(); return;} if(!items.length){editImagePreview.innerHTML=''; editImagePreview.hidden=true; renderExistingImageInputs(); syncEditCatalogImagesFromRetained(); return;} editImagePreview.hidden=false; editImagePreview.innerHTML=items.map((item,index)=>`<div class="product-image-tile group relative aspect-square overflow-hidden rounded-xl border border-[#E2E8F0] bg-[#F8FAFC]"><img src="${item.url}" alt="${escapeHtml(item.name||currentProduct?.name||'Product image')}" class="h-full w-full object-cover">${index===0?'<span class="absolute bottom-2 left-2 rounded-full bg-white/95 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wide text-[#3F51B5] shadow-sm">Main photo</span>':''}${item.kind==='new'?'<span class="absolute left-2 top-2 rounded-full bg-[#0F172A]/75 px-2 py-0.5 text-[10px] font-semibold text-white">New</span>':''}<button type="button" class="edit-remove-image absolute right-2 top-2 inline-flex h-7 w-7 items-center justify-center rounded-full bg-[#0F172A]/75 text-xs font-bold text-white" data-kind="${item.kind}" data-index="${item.kind==='existing'?index:item.index}" aria-label="Remove image">&times;</button></div>`).join(''); renderExistingImageInputs(); document.querySelectorAll('.edit-remove-image').forEach((button)=>button.addEventListener('click',()=>{if(button.dataset.kind==='existing'){const existingIndex=Number(button.dataset.index); const existingItems=(currentProduct?.image_paths||[]).filter((path)=>retainedExistingImages.includes(path)); retainedExistingImages=existingItems.filter((_,idx)=>idx!==existingIndex);}else{lastRemovedPendingImageIndex=Number(button.dataset.index);selectedEditImages=selectedEditImages.filter((_,idx)=>idx!==lastRemovedPendingImageIndex); syncSelectedFiles(editImageInput,selectedEditImages);} renderEditImages();})); syncEditCatalogImagesFromRetained();};
...
editApplyBulkValues?.addEventListener('click',()=>{const q=editBulkStock.value;if(q===''){window.alert('Enter a stock quantity to apply to every variant.');return;}if(!hasMultipleVariants()){return;}const amt=String(Math.max(0,parseInt(String(q),10)||0));editRows=editRows.map((r)=>({...r,stock:amt}));setApplySameStockMode(amt);clearStockCarryNotice();renderVariantRows();});editApplyBasePriceToVariants?.addEventListener('click',()=>{if(!hasMultipleVariants()){return;}const price=editPrice?.value||'';if(price===''){window.alert('Set a base price first.');return;}editRows=editRows.map((r)=>({...r,price}));renderVariantRows();});
editDistributeEqualBtn?.addEventListener('click',()=>{const raw=editDistributeTotal?.value||pendingStockCarryTotal||'';const total=parseInt(String(raw),10);if(Number.isNaN(total)||total<0){window.alert('Enter a whole number total stock to split across variants.');return;}const n=editRows.length;if(n<1){return;}if(n<2){window.alert('Add option groups so there are at least two variants.');return;}splitStockAcrossVariants(total);});editStockCarrySplitBtn?.addEventListener('click',()=>{const total=pendingStockCarryTotal ?? parseInt(String(editDistributeTotal?.value||'0'),10);if(!total){window.alert('No previous stock total was found to split.');return;}splitStockAcrossVariants(total);});editStockCarryDismissBtn?.addEventListener('click',()=>{clearStockCarryNotice();});
