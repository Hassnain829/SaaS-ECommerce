    ...
    <div class="relative flex max-h-[94vh] w-full max-w-5xl flex-col overflow-hidden rounded-3xl border border-[#E2E8F0] bg-white shadow-2xl">
        ...
        <div class="overflow-y-auto px-6 py-6">
            ...
            <form id="editProductForm" method="POST" enctype="multipart/form-data" class="space-y-6">
                ...
                <input type="hidden" name="store_id" id="edit_product_store_id" value="{{ old('store_id', '') }}">
                ...
                <div class="rounded-[24px] border border-[#DDE7F3] bg-white p-5 shadow-sm sm:p-7">
                    ...
                    <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                        <div>
                            <label for="edit_product_store_select" class="mb-2 block text-sm font-medium text-[#334155] font-[Poppins]">Store</label>
                            <select id="edit_product_store_select" class="w-full rounded-lg border border-[#CBD5E1] bg-white px-4 py-3 text-sm text-[#0F172A]">
    ...
    <div class="w-full max-w-lg overflow-hidden rounded-3xl border border-[#FECACA] bg-white shadow-2xl">
        ...
        <div class="px-6 pb-6 pt-2">
            ...
            <form id="deleteProductForm" method="POST" class="mt-6">
                @csrf
                @method('DELETE')
                <input type="hidden" name="store_id" id="delete_product_store_id" value="">
...
const deleteWarningModal=document.getElementById('deleteProductWarningModal'); const openDeleteWarning=document.getElementById('openDeleteProductWarning'); const cancelDeleteProduct=document.getElementById('cancelDeleteProduct'); const deleteProductName=document.getElementById('deleteProductName'); const deleteProductStoreId=document.getElementById('delete_product_store_id');
const editProductId=document.getElementById('edit_product_id'); const editStoreId=document.getElementById('edit_product_store_id'); const editStoreSelect=document.getElementById('edit_product_store_select'); const editTypeSelect=document.getElementById('edit_product_type_select'); const editTypeValue=document.getElementById('edit_product_type_value'); const editCustomTypeWrap=document.getElementById('editProductCustomTypeWrap'); const editCustomType=document.getElementById('edit_product_custom_type'); const editCustomTypeHidden=document.getElementById('edit_product_custom_type_hidden');
...
closeButtons.forEach((b)=>b?.addEventListener('click',closeAll)); editStoreSelect?.addEventListener('change',()=>{editStoreId.value=editStoreSelect.value||'';}); editTypeSelect?.addEventListener('change',syncType); editCustomType?.addEventListener('input',syncType); editImageInput?.addEventListener('change',()=>{const incomingFiles=Array.from(editImageInput.files||[]); if(incomingFiles.length){selectedEditImages=[...selectedEditImages,...incomingFiles]; syncSelectedFiles(editImageInput,selectedEditImages);} renderEditImages();});
...
openDeleteWarning?.addEventListener('click',()=>{if(!currentProduct) return; deleteProductName.textContent=currentProduct.name||'this product'; deleteProductStoreId.value=editStoreSelect.value||currentProduct.store_id||''; deleteWarningModal.classList.remove('hidden'); deleteWarningModal.classList.add('flex');});
