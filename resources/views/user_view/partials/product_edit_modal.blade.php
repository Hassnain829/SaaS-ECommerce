        ...
        <div @class([
            ...
            <form id="editProductForm" method="POST" enctype="multipart/form-data" class="space-y-6" @if ($editFormAction) action="{{ $editFormAction }}" @endif data-product-store-url="{{ route('product.store') }}" data-product-update-url-template="{{ route('product.update', ['productId' => '__PRODUCT_ID__']) }}">
                ...
                <div class="{{ $editSectionClass }}" @if ($productEditSurface === 'page' && $productEditPageNative) id="catalog-edit-section-basics" @endif>
                    ...
                    <div id="catalog-edit-section-pricing" @class(['mt-6 scroll-mt-28 grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-4' => $productEditSurface === 'page' && $productEditPageNative, 'mt-6 grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-4' => ! ($productEditSurface === 'page' && $productEditPageNative)])>
                        ...
                        <div id="editSimpleStockWrap">
                            ...
                            <input id="edit_product_stock" type="number" min="0" step="1" value="{{ old('bulk_stock', '0') }}" class="w-full rounded-lg border border-[#CBD5E1] px-4 py-3 text-sm text-[#0F172A]" inputmode="numeric">
                            <p class="mt-1 text-xs text-[#64748B]">Units available to sell. This is inventory — not the low-stock alert.</p>
                        </div>
                        <div id="editMultiStockHint" class="hidden rounded-lg border border-[#E2E8F0] bg-[#F8FAFC] px-4 py-3">
                            <p class="text-sm font-medium text-[#0F172A]">Stock is per variant</p>
                            <p class="mt-1 text-xs text-[#64748B]">Set quantity on each row in the Inventory section below.</p>
                        </div>
                        <div><label for="edit_product_stock_alert" class="mb-2 block text-sm font-medium text-[#334155]">Low stock alert</label><input id="edit_product_stock_alert" name="stock_alert" type="number" min="0" step="1" value="{{ old('stock_alert', '0') }}" class="w-full rounded-lg border border-[#CBD5E1] px-4 py-3 text-sm text-[#0F172A]"><p class="mt-1 text-xs text-[#64748B]">Warn when stock falls to this number or below. Does not change inventory.</p></div>
...
const hasMultipleVariants=()=>editRows.length>1;const stockDisplayValue=(value)=>String(value ?? '');const syncSimpleStockFromRows=()=>{if(!editProductStock)return;if(isSimpleInventoryProduct()&&editRows.length){editProductStock.value=stockDisplayValue(editRows[0].stock ?? 0);}else if(!hasMultipleVariants()){editProductStock.value=stockDisplayValue(editProductStock.value||0);}};const syncRowsFromSimpleStock=()=>{if(!editProductStock||!isSimpleInventoryProduct()||!editRows.length)return;const qty=String(Math.max(0,parseInt(String(editProductStock.value??'0'),10)||0));editRows[0]={...editRows[0],stock:qty};};const updateTotalStockDisplay=()=>{const total=editRows.reduce((sum,row)=>sum+Math.max(0,parseInt(String(row.stock??''),10)||0),0);if(editTotalStockDisplay){editTotalStockDisplay.textContent=String(total);}if(editBulkPrice&&editPrice){editBulkPrice.value=editPrice.value||'';}syncSimpleStockFromRows();};const updateInventoryToolsVisibility=()=>{syncSimpleRowPriceFromBase();const multi=hasMultipleVariants();if(editInventoryToolsPanel){editInventoryToolsPanel.classList.toggle('hidden',!multi);}if(editSimpleStockWrap){editSimpleStockWrap.classList.toggle('hidden',multi);}if(editMultiStockHint){editMultiStockHint.classList.toggle('hidden',!multi);}if(!multi){clearStockCarryNotice();}else if(pendingStockCarryTotal&&editStockCarryNotice){editStockCarryNotice.classList.remove('hidden');}if(editInventorySectionTitle){editInventorySectionTitle.textContent=multi?'Sellable combinations (variants)':'Inventory';}if(editInventorySectionLead){editInventorySectionLead.textContent=multi?'Each row is one combination. Set a different price and stock on every variant when needed. Total stock is the sum of all variant stocks.':'Stock for this product. Base price above is the default selling price.';}if(editPreviewSection){editPreviewSection.classList.toggle('hidden',!multi);}updateTotalStockDisplay();};
...
closeButtons.forEach((b)=>b?.addEventListener('click',closeAll)); editPrice?.addEventListener('input',()=>{syncSimpleRowPriceFromBase();if(isSimpleInventoryProduct()){renderVariantRows();}});editPrice?.addEventListener('change',()=>{syncSimpleRowPriceFromBase();if(isSimpleInventoryProduct()){renderVariantRows();}});editProductStock?.addEventListener('input',()=>{syncRowsFromSimpleStock();if(isSimpleInventoryProduct()){renderVariantRows();}else{updateTotalStockDisplay();}});editProductStock?.addEventListener('change',()=>{syncRowsFromSimpleStock();if(isSimpleInventoryProduct()){renderVariantRows();}});editTypeSelect?.addEventListener('change',syncType); editCustomTypeInput?.addEventListener('input',syncType); editCustomTypeBehavior?.addEventListener('change',syncType); editImageInput?.addEventListener('change',()=>{const incomingFiles=Array.from(editImageInput.files||[]); if(incomingFiles.length){selectedEditImages=[...selectedEditImages,...incomingFiles]; syncSelectedFiles(editImageInput,selectedEditImages);} renderEditImages();});
