...
const setSellingMode=(mode)=>{if(mode==='single'&&hasOptionGroups()){const applySingleItem=()=>{syncEditRowsFromDom();const total=getEditRowsStockTotal(editRows);const emptyRow=editRows.find(isEmptyOptionMap);const first=emptyRow||editRows[0]||{};includeStandardBase=false;sellingMode='single';editVariationTypes=[];editRows=[makeInventoryRow({id:emptyRow?.id??'',sku:first.sku||'',price:'',compare_at_price:first.compare_at_price||defaultCompareAtValue(),stock:String(total),stock_alert:first.stock_alert??editStockAlert?.value??0,shipping_weight:first.shipping_weight||'',product_image_id:first.product_image_id||'',product_image_ids:Array.isArray(first.product_image_ids)?first.product_image_ids.slice():(first.product_image_id?[first.product_image_id]:[]),custom_fields:cloneCustomFields(first)})];if(editProductStock){editProductStock.value=String(total);}renderVariationInputs();renderVariationCards();renderVariantRows({skipDomSync:true});};const ask=window.MerchantUi&&typeof window.MerchantUi.confirm==='function'?window.MerchantUi.confirm({title:'Switch to a single item?',body:'Switching to Single item removes sizes, colors, and variant rows. Stock is combined into one amount.',warningLabel:'Variants will be removed',warningBody:'Option groups and per-variant rows are combined into one stock amount.',cancelLabel:'Keep variants',confirmLabel:'Switch to single item',tone:'warning'}):null;if(ask){ask.then((ok)=>{if(!ok){updateSellingModeUi();return;}applySingleItem();});return;}if(!window.confirm('Switching to Single item removes sizes, colors, and variant rows. Stock is combined into one amount.')){updateSellingModeUi();return;}applySingleItem();return;}sellingMode=mode;if(mode==='multiple'){renderVariationCards();updateInventoryToolsVisibility();return;}updateSellingModeUi();};
