...
closeButtons.forEach((b)=>b?.addEventListener('click',closeAll)); editPrice?.addEventListener('input',()=>{syncSimpleRowPriceFromBase();if(isSimpleInventoryProduct()){renderVariantRows();}});editPrice?.addEventListener('change',()=>{syncSimpleRowPriceFromBase();if(isSimpleInventoryProduct()){renderVariantRows();}});editTypeSelect?.addEventListener('change',syncType); editCustomTypeInput?.addEventListener('input',syncType); editCustomTypeBehavior?.addEventListener('change',syncType); editImageInput?.addEventListener('change',()=>{const incomingFiles=Array.from(editImageInput.files||[]); if(incomingFiles.length){selectedEditImages=[...selectedEditImages,...incomingFiles]; syncSelectedFiles(editImageInput,selectedEditImages);} renderEditImages();});
