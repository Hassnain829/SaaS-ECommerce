@php
    ...
    $tags = $embedCatalogHubs
        ? $managementTags
        : ($managementTags->isNotEmpty() ? $managementTags : ($tags ?? collect()));
...
<div id="tagHubModal" class="@if ($embedCatalogHubs) flex min-h-0 w-full flex-1 flex-col overflow-hidden @else ui-modal-shell ui-modal-shell--clear {{ $openTagHub ? 'flex' : 'hidden' }} @endif">
    ...
    <div class="@if ($embedCatalogHubs) relative flex max-h-full min-h-0 w-full flex-1 flex-col overflow-hidden rounded-xl border border-[#E2E8F0] bg-white shadow-sm @else ui-modal-panel ui-modal-panel--md @endif">
        <div class="flex shrink-0 items-start justify-between gap-3 border-b border-[#E2E8F0] bg-white px-4 py-3.5 sm:px-5">
            <div class="min-w-0">
                <h2 class="text-base font-semibold tracking-tight text-[#0F172A]">Tags</h2>
                @unless ($embedCatalogHubs)
                    <p class="mt-0.5 text-xs text-[#64748B]">Labels like Featured, Sale, or seasonal drops.</p>
                @endunless
            </div>
            <div class="flex shrink-0 items-center gap-2">
                <button type="button" id="tag-hub-open-add" class="inline-flex items-center gap-1.5 rounded-lg bg-brand px-3 py-2 text-xs font-semibold text-white shadow-sm transition hover:bg-brand-hover focus:outline-none focus:ring-2 focus:ring-[#0052CC]/35">
        ...
        <div class="min-h-0 flex-1 overflow-y-auto overscroll-contain px-4 py-3 sm:px-5 sm:py-3">
            <table class="w-full min-w-[300px] text-left text-sm text-[#334155]">
                ...
                <tbody id="catalog-hub-tags-rows" class="divide-y divide-[#F1F5F9]" data-catalog-hub-rows="tags">
                    @forelse ($tags as $tag)
                        @php
                        ...
                        @endphp
                        <tr class="align-middle transition-colors hover:bg-[#F8FAFC]/90" data-catalog-row="tag" data-catalog-row-id="{{ $tag->id }}">
                    ...
                    @empty
                        <tr data-catalog-hub-empty="tags">
                            <td colspan="4" class="px-2 py-10 text-center sm:px-4">
                                ...
                                <button type="button" id="tag-hub-empty-add" class="mt-4 inline-flex items-center gap-1.5 rounded-lg bg-brand px-4 py-2 text-xs font-semibold text-white shadow-sm hover:bg-brand-hover">Add tag</button>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
            ...
            <details id="tag-hub-add-details" class="mt-5 rounded-xl border border-[#E2E8F0] bg-white px-4 py-3 shadow-sm sm:px-4 sm:py-3.5" @if ($reopenAdd) open @endif>
                ...
                <div class="mt-4 border-t border-[#F1F5F9] pt-4">
                    ...
                    @endif
                    <form method="POST" action="{{ route('tags.store') }}" class="space-y-3" id="tagHubAddForm" data-catalog-kind="tag" data-turbo="false">
                        @csrf
                        ...
                        <div class="flex justify-end pt-1">
                            <button type="submit" class="rounded-lg bg-brand px-5 py-2.5 text-sm font-semibold text-white shadow-sm transition hover:bg-brand-hover focus:outline-none focus:ring-2 focus:ring-[#0052CC]/35">Save tag</button>
                        </div>
                    </form>
                </div>
            </details>
...
<div id="tagEditModal" class="ui-modal-shell ui-modal-shell--clear ui-modal-shell--nested {{ $reopenEdit ? 'flex' : 'hidden' }}">
    ...
    <div class="ui-modal-panel ui-modal-panel--md">
        <div class="flex items-center justify-between border-b border-[#E2E8F0] px-4 py-2.5">
            <h2 class="text-sm font-semibold text-[#0F172A]">Edit <span id="tagEditTitleName" class="text-[#475569]">{{ $reopenEdit ? $editingTag->name : '…' }}</span></h2>
        ...
        <div class="ui-modal-body !px-4 !py-3">
            ...
            @endif
            <form method="POST" action="{{ $reopenEdit ? route('tags.update', $editingTag) : '#' }}" id="tagEditForm" class="space-y-2" data-catalog-kind="tag" data-turbo="false">
                @csrf
                @include('user_view.partials.catalog_tools_return_field')
                @method('PATCH')
...
<div id="tagDeleteWarningModal" class="ui-modal-shell ui-modal-shell--alert hidden">
    <div class="ui-modal-panel ui-modal-panel--sm border-[#FECACA]">
        <div class="px-4 pb-3 pt-4">
            <h3 class="text-sm font-semibold text-[#0F172A]">Remove <span id="deleteTagName" class="text-[#475569]"></span>?</h3>
            <p class="mt-1.5 text-xs text-[#64748B]">Products keep their catalog data; this label is removed from them.</p>
        </div>
        <div class="px-4 pb-4 pt-0">
            <form id="deleteTagForm" method="POST" class="mt-2" data-catalog-kind="tag" data-turbo="false">
                @csrf
                @include('user_view.partials.catalog_tools_return_field')
                @method('DELETE')
