...
<body class="user-typography bg-white antialiased min-h-screen flex flex-col overflow-x-hidden font-[Inter]">
                ...
                <form class="space-y-6" method="POST" action="{{ route('signin.attempt') }}">
                    ...
                    <div class="space-y-2">
                        ...
                        <div class="relative">
                            ...
                            <input id="signin_password" type="password" name="password" placeholder="********" class="w-full pl-11 pr-16 py-3.5 bg-[#F8FAFC] border border-[#E2E8F0] rounded-xl text-sm text-[#334155] placeholder:text-[#6B7280] focus:outline-none focus:ring-2 focus:ring-[#0052CC]/20 focus:border-[#0052CC] transition">
                            <button type="button" class="absolute inset-y-0 right-3 text-xs font-semibold text-[#64748B]" data-password-toggle="signin_password" aria-label="Show password" aria-pressed="false">Show</button>
                        </div>
                    </div>
    ...
    <footer class="border-t border-gray-200 py-5 px-4 text-center text-sm text-[#94A3B8] font-inter font-medium">
        <div class="max-w-7xl mx-auto flex flex-wrap justify-center items-center gap-x-4 gap-y-1">
            ...
            <span class="hidden sm:inline">&middot;</span>
            <a href="{{ route('legal.terms') }}" class="hover:text-[#64748B] transition">Terms of Service</a>
        </div>
    </footer>
</body>
</html>
