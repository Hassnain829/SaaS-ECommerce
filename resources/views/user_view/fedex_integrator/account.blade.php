...
@section('content')
    @php
        ...
        $defaultCountry = old('country_code', $prefill['country_code'] ?? ($defaultCountry ?? 'US'));
    ...
    <div class="mx-auto max-w-[900px] space-y-6">
        ...
        <section class="rounded-2xl border border-[#CBD5E1] bg-white p-5 shadow-sm">
            ...
            <form method="POST" action="{{ route('settings.shipping.fedex-integrator.account.submit', $session) }}" class="mt-5 grid gap-4 sm:grid-cols-2">
                @csrf
                <label class="sm:col-span-2 block space-y-1"><span class="text-xs font-semibold text-[#64748B]">FedEx account number</span><input name="provider_account_number" required value="{{ old('provider_account_number', $prefill['account_number'] ?? '') }}" class="h-10 w-full rounded-lg border border-[#CBD5E1] px-3 text-sm" inputmode="numeric" maxlength="9"></label>
                ...
                <label class="block space-y-1"><span class="text-xs font-semibold text-[#64748B]">State / province</span><input name="state" required maxlength="2" value="{{ old('state', $prefill['state'] ?? '') }}" class="h-10 w-full rounded-lg border border-[#CBD5E1] px-3 text-sm uppercase" placeholder="TX or ON"></label>
                <label class="block space-y-1"><span class="text-xs font-semibold text-[#64748B]">Postal / ZIP code</span><input name="postal_code" required value="{{ old('postal_code', $prefill['postal_code'] ?? '') }}" class="h-10 w-full rounded-lg border border-[#CBD5E1] px-3 text-sm" placeholder="10001 or A1A 1A1"></label>
