...
@php
    ...
    $storeErrorKeys = [
        'name', 'contact_email', 'address', 'currency', 'timezone', 'category', 'business_models',
        'custom_category', 'store_logo', 'default_location_id', 'confirm_currency_conversion',
    ];
...
@section('topbar')
    <x-ui.merchant-topbar title="Settings" lead="Store and account preferences for this workspace.">
        <x-slot:actions>
            @if ($settingsTab === 'store' && $canManageStoreSettings && $store)
                <button
                    type="submit"
                    form="general-settings-store-form"
                    class="inline-flex items-center rounded-md bg-brand px-3.5 py-2 text-sm font-semibold text-white transition hover:bg-brand-hover"
                >
                    Save store settings
                </button>
            @endif
        ...
        @if ($settingsTab === 'store')
            ...
            @else
                <section class="gs-info-banner" role="note">
                    ...
                    <p>
                        These store defaults are visible to your role as read-only facts. Ask a store owner to change them.
                        Past order amounts and currencies are never rewritten when settings change.
                    </p>
                </section>
