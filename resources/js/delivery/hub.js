...
(function () {
    ...
    function zoneStoreUrl() {
        const page = shippingPage();
        return page ? (page.getAttribute('data-zone-store-url') || '') : '';
    }

    function methodStoreUrl() {
        const page = shippingPage();
        return page ? (page.getAttribute('data-method-store-url') || '') : '';
    }
    ...
    function readJson(id, fallback) {
        try {
            ...
            return JSON.parse(el.textContent || (Array.isArray(fallback) ? '[]' : '{}'));
        } catch (e) {
            return fallback;
        }
    }
    ...
    function regionCatalog() {
        return readJson('delivery-region-catalog', {});
    }

    function fedExCatalog() {
        return readJson('fedex-services-catalog', []);
    }
    ...
    function renderRegionMulti(countryCode, selectedRegions) {
        ...
        if (! countryCode) {
            ...
            html += '<p class="rounded-lg border border-[#E2E8F0] bg-[#F8FAFC] px-3 py-2 text-xs text-[#64748B]">This country has no predefined states or provinces. Leave Entire country on, or add postal coverage.</p>';
        } else {
    ...
    function handleDocumentClick(event) {
        ...
        const availability = event.target.closest('[data-availability-toggle]');
        if (availability) {
        ...
        if (openBtn) {
            handleOpenDrawer(openBtn);
            return;
        }

        const zoneEdit = event.target.closest('.zone-edit-btn');
        if (zoneEdit) {
            handleZoneEdit(zoneEdit);
            return;
        }

        const methodEdit = event.target.closest('.method-edit-btn');
        if (methodEdit) {
            handleMethodEdit(methodEdit);
            return;
        }

        const entireCountry = event.target.closest('#zone-entire-country');
        if (entireCountry) {
            const next = ! entireCountry.classList.contains('is-on');
            setEntireCountry(next, []);
            return;
        }

        const regionClear = event.target.closest('[data-region-action="clear"]');
        if (regionClear) {
        ...
        const postalBuilder = event.target.closest('#zone-postal-builder');
        if (postalBuilder) {
            const list = postalBuilder.querySelector('[data-postal-rules-list]');
            if (event.target.closest('[data-postal-rule-add]')) {
                const empty = postalBuilder.querySelector('[data-postal-rules-empty]');
                if (empty) {
                    empty.remove();
                }
                if (list) {
            ...
            const removeBtn = event.target.closest('[data-postal-rule-remove]');
            if (removeBtn) {
                const row = removeBtn.closest('[data-postal-rule-row]');
                if (row) {
                    row.remove();
                }
                if (list && ! list.querySelector('[data-postal-rule-row]')) {
                    list.innerHTML = '<p class="rounded-lg border border-dashed border-[#CBD5E1] bg-white px-3 py-2 text-xs text-[#94A3B8]" data-postal-rules-empty>No postal rules — entire selected geography applies.</p>';
                }
                syncPostalRulesJson(postalBuilder);
                return;
            }
        }

        const switchBtn = event.target.closest('#method-available-switch, #fedex-services-available-switch');
        if (switchBtn) {
            const checkboxId = switchBtn.id === 'method-available-switch' ? 'method-field-available' : 'fedex-services-available';
    ...
    function handleDocumentChange(event) {
        ...
        if (target.id === 'zone-field-country') {
            ...
            if (! entireOn) {
                renderRegionMulti(target.value || '', []);
            }
            return;
        }
        if (target.matches('[data-method-price-mode]')) {
            setMethodPriceMode(target.value);
            return;
        }
        if (target.matches('[data-postal-rule-type]')) {
            syncPostalRulesJson(target.closest('#zone-postal-builder'));
        }
    }
    ...
    function handleDocumentInput(event) {
        ...
        if (target.id === 'method-field-flat') {
            syncMethodFlatMirror();
            return;
        }
        if (target.id === 'method-field-flat-mirror') {
            const flatInput = document.getElementById('method-field-flat');
            if (flatInput) {
                flatInput.value = target.value;
            }
            return;
        }
        if (target.matches('[data-postal-rule-value]')) {
            syncPostalRulesJson(target.closest('#zone-postal-builder'));
        }
    }
    ...
    function handleDocumentSubmit(event) {
        ...
        if (form.classList.contains('shipping-submit-form')) {
    ...
    const bindOnce = () => {
        if (window.__deliveryHubDocBound) {
            return;
        }
        window.__deliveryHubDocBound = true;
        ...
        document.addEventListener('turbo:before-cache', () => {
            ...
            const page = shippingPage();
            if (page) {
                delete page.dataset.deliveryHubBound;
            }
        });
    };
    ...
    const boot = () => {
        ...
        bindOnce();
        if (page.dataset.deliveryHubBound === '1') {
            return;
        }
