...
| R-005 | inventory | Double stock deduction on duplicate external orders | High | Low | Critical | Reservation service uses transactions/locks | **Step 3:** dedup identity required; `external_order_id` unique index | Monitor integrator adoption | **Mitigated** |
...
| R-012 | external sync | Integrator retries create duplicate orders | High | Low | High | Idempotency-Key + external_order_number/id dedup when provided | **Step 3C:** `external_order_id` or `external_order_number` required; Idempotency-Key alone rejected | Document API contract for partners | **Mitigated** |
...
| R-018 | shipping/delivery | Wrong origin selected for address | High | Low | High | `FulfillmentOriginRouter` service-area + stock priority | **Step 3:** 9 hardening tests added | Extend matrix as carriers attach | **Mitigated for 6C-0A** |
