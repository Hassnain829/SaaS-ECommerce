...
class Phase35StoreSettingsAlignmentTest extends TestCase
{
    ...
    public function test_store_settings_and_onboarding_copy_separate_locations_markets_currency_and_timezone(): void
    {
        ...
        $this->actingAs($owner)
            ...
            ->assertSee('Name, contact email, logo, and business address for this store.', false)
            ...
            ->assertSee('Labels catalog prices and new activity.', false)
            ->assertSee('Default store timezone', false)
            ->assertSee('Controls how dates are displayed.', false)
            ->assertDontSee('Primary Market', false)
            ->assertDontSee('Manage via region settings.', false)
            ->assertSee('Save store settings', false)
            ->assertDontSee('Edit profile', false)
            ->assertDontSee('Configure', false)
            ->assertDontSee('Edit Store', false)
            ->assertSee(route('generalSettings', ['tab' => 'store']), false)
            ->assertSee(route('generalSettings', ['tab' => 'account']), false)
            ->assertDontSee('editable later', false)
            ...
            ->assertDontSee('Locations are for stock, not selling rules', false)
            ->assertDontSee('Branding colors', false)
            ->assertSee('Read-only fact', false)
            ->assertDontSee('Live workspace', false)
            ->assertSee('Setup status', false);
    ...
    public function test_shipping_page_is_real_setup_and_has_no_fake_preview_controls(): void
    {
        ...
        $this->actingAs($owner)
            ...
            ->assertOk()
            ->assertSee('Delivery setup', false)
            ->assertSee('Manage ship-from locations, delivery areas, checkout delivery options, and optional delivery providers.', false)
            ->assertSee('Delivery areas', false)
            ->assertSee('Delivery options', false)
            ->assertSee('FedEx Merchant Account', false)
            ->assertSee('Fulfillment locations', false)
            ->assertSee('Add delivery area', false)
            ->assertSee('Connect FedEx account', false)
            ->assertSee('Carrier routing automation and advanced fulfillment workflows will be added in later phases.', false)
